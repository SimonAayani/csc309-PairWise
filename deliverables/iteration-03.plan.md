...
>* **Management of Developer Roles:**
>Proper handling of roles and responsibilities is a main priority in this deliverable, as our strategy to delegate multiple group members to a single role in the previous deliverable was not as successful as it could have been. Although splitting up frontend tasks by the different user pages may have been a good idea, the strategy was not successful because the members were not provided clear enough specifications about how to implement the user page they were given. This time, we will pay more attention to settling specifications of our webpages before assigning tasks. All group members will have input into how webpage features are laid out before the task is assigned. We will try to assign each developer to a similar part of the product as they were working on previously; for instance, frontend developers will tend to stay frontend developers, and backend developers will continue to work on backend. However, all developers will become more aware of the specifications of each component, and will thus be better able to integrate multiple components or help out with another developer's work if need be. Specifications for each task include which software/tools to build with, page layouts, functionalities, and relationship to other pages.

>* **Meetings:**
>In the last deliverable we found having in-person coding sessions over the weekends to be quite useful as we were able to ask questions to each other and solve code bugs quicker. However we were not able to have enough coding sessions as we wanted to. We are planning to have more in-person coding sessions spread throughout the next few weeks to ensure that group members have ample time to discuss/assign tasks, work on tasks, and share progress. If possible, we also aim to have a few coding sessions on weekdays if the group is able to attend.

>* **Create a Measurable Organization System:**
>It would be unreasonable to assign major tasks like implementing a search algorithm, or connecting frontend with backend to a single group member to complete. Not only does it risk failure to complete the task, but also does not allow enough time for learning. So we have decided to create a step-by-step system that outlines the small steps and processes needed to be completed to ensure the desired outcome. Group members will be assigned smarter and measurable tasks, and will be given earlier deadlines for said tasks spread throughout the next few weeks. This process will also help avoid the problems we had in the previous iteration, in which everyone made plans to complete their features by the very end, leading us to struggle to put different and sometimes incompatible components together with almost no time left. With intermediate deadlines, we will have a steady flow of development of new features; this process will allow time for any errors/problems faced or learning curves and avoid last minute work, while still keeping the project on track.

#### Roles & responsibilities

>* **Project Supervisors:**
>Supervisors have the responsibility of collecting group decisions from the planning meeting and keeping track of the product’s progress. They will collect and maintain information about tasks, including tasks that have been assigned to group members, completed and incomplete tasks. Expectations of the Supervisor is to host weekly meetings to review each individual’s progress and check if they have been keeping up with their deadlines, discuss solutions/problems that arose, and re-distribute tasks if necessary. Only one member of the team will be assigned to this role each week; it will switch from one week to the next, to give multiple people experience with a more organizational side of the software process.
...
>* Contributor: Alex Hurka (ahurka), Simon Aayani (SimonAayani)
...
> The review meeting will be held on the afternoon of Saturday, March 31st. At this point we plan to have the PairWise application completed according to our specifications. During the review meeting we will mainly discuss the success of the process used in previous iterations and what we can take away from this final stage of the project. During and/or after the review meeting, each developer will be briefed on the video script and plan, and will prepare for filming the product demonstration. The remainder of the day will be spent filming and editing the demo video.
...
>* Allow a user to create a new account, which contains basic information such as names, password, and email, that is stored in the database. This information will be used to allow the users to authenticate later on, and to associate user profiles, searches, groups, and messages with the user that created them.
...
>   * As a new member of a group, I want to be able to schedule meetings with the other group members, so the meetings can happen at times when I am available.
